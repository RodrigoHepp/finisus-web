...
import { FormularioItemDialogComponent } from '../formulario-item/formulario-item.dialog';
...
@Component({
  ...
  styleUrl: './lista-itens.page.scss',
...
export class ListaItensPage {
  ...
  protected readonly dados = signal<PaginaResponse<Item>>(paginaVazia());
  protected readonly carregando = signal(true);
  protected readonly erro = signal<string | null>(null);
  protected readonly excluindo = signal(false);
  protected readonly colunas = ['nome', 'categoria', 'acoes'];
  constructor() {
    effect(() => this.carregar({ ...PAGINACAO_PADRAO, pagina: this.paginaDaRota() }));
  }
  protected abrirFormulario(item?: Item): void {
    this.dialog
      .open(FormularioItemDialogComponent, {
        data: { item },
  ...
  protected excluir(item: Item): void {
    if (this.excluindo()) {
      return;
    }
    ...
    this.dialog
      ...
      .pipe(
        switchMap((confirmado: boolean | undefined) => {
          ...
          return this.api.inativar(item.id).pipe(
      ...
      .subscribe(() => {
        this.mensagens.sucesso(
          this.translate.instant('CADASTROS.MENSAGENS.EXCLUIDO_COM_SUCESSO', { nome: item.nome }),
        );
        this.carregar({ pagina: this.dados().pagina, tamanho: this.dados().tamanho });
      });
  }
...
function paginaVazia(): PaginaResponse<Item> {
