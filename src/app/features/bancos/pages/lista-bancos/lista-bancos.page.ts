...
import { FormularioBancoDialogComponent } from '../formulario-banco/formulario-banco.dialog';
...
@Component({
  ...
  styleUrl: './lista-bancos.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ListaBancosPage {
  private readonly api = inject(BancosApiService);
  private readonly dialog = inject(MatDialog);
  private readonly mensagemGlobal = inject(MensagemGlobalService);
  ...
  protected readonly dados = signal<PaginaResponse<Banco>>(paginaVazia());
  protected readonly carregando = signal(true);
  protected readonly erro = signal<string | null>(null);
  protected readonly excluindo = signal(false);
  protected readonly colunas = ['nome', 'codigo', 'acoes'];

  constructor() {
    effect(() => this.carregar({ ...PAGINACAO_PADRAO, pagina: this.paginaDaRota() }));
  }

  protected abrirFormulario(banco?: Banco): void {
    this.dialog
      .open(FormularioBancoDialogComponent, {
        data: { banco },
  ...
  protected excluir(banco: Banco): void {
    if (this.excluindo()) {
      return;
    }
    ...
    this.dialog
      ...
      .pipe(
        switchMap((confirmado: boolean | undefined) => {
          ...
          return this.api.inativar(banco.id).pipe(
            finalize(() => this.excluindo.set(false)),
            catchError((erro: unknown) => {
              this.mensagemGlobal.erro(this.mensagemDeErro(erro));
              return EMPTY;
            }),
          );
        }),
      ...
      .subscribe(() => {
        this.mensagemGlobal.sucesso(
          this.translate.instant('CADASTROS.MENSAGENS.EXCLUIDO_COM_SUCESSO', { nome: banco.nome }),
        );
        this.carregar({ pagina: this.dados().pagina, tamanho: this.dados().tamanho });
      });
  }
  ...
  private mensagemDeErro(erro: unknown): string {
    ...
    return this.translate.instant('CADASTROS.MENSAGENS.ERRO_CARREGAR');
  }
}

function paginaVazia(): PaginaResponse<Banco> {
