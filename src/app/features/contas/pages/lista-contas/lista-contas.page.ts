...
import { FormularioContaDialogComponent } from '../formulario-conta/formulario-conta.dialog';
...
@Component({
  ...
  styleUrl: './lista-contas.page.scss',
...
export class ListaContasPage {
  ...
  protected readonly dados = signal<PaginaResponse<Conta>>(paginaVazia());
  protected readonly carregando = signal(true);
  protected readonly erro = signal<string | null>(null);
  protected readonly excluindo = signal(false);
  protected readonly colunas = ['nome', 'tipo', 'saldo', 'acoes'];

  constructor() {
    effect(() => this.carregar({ ...PAGINACAO_PADRAO, pagina: this.paginaDaRota() }));
  }

  protected abrirFormulario(conta?: Conta): void {
    this.dialog
      .open(FormularioContaDialogComponent, {
        data: { conta },
  ...
  protected excluir(conta: Conta): void {
    if (this.excluindo()) {
      return;
    }
    ...
    this.dialog
      ...
      .pipe(
        switchMap((confirmado: boolean | undefined) => {
          ...
          return this.api.inativar(conta.id).pipe(
      ...
      .subscribe(() => {
        this.mensagemGlobal.sucesso(
          this.translate.instant('CADASTROS.MENSAGENS.EXCLUIDO_COM_SUCESSO', { nome: conta.nome }),
        );
        this.carregar({ pagina: this.dados().pagina, tamanho: this.dados().tamanho });
      });
  }
...
function paginaVazia(): PaginaResponse<Conta> {
