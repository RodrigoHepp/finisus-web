...
import { MeioPagamento } from '../../meio-pagamento.model';
import { MeiosPagamentoApiService } from '../../meios-pagamento-api.service';
import { FormularioMeioPagamentoDialogComponent } from '../formulario-meio-pagamento/formulario-meio-pagamento.dialog';
...
@Component({
  ...
  templateUrl: './lista-meios-pagamento.page.html',
  styleUrl: './lista-meios-pagamento.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ListaMeiosPagamentoPage {
  ...
  protected readonly dados = signal<PaginaResponse<MeioPagamento>>(paginaVazia());
  protected readonly carregando = signal(true);
  protected readonly erro = signal<string | null>(null);
  protected readonly excluindo = signal(false);
  protected readonly colunas = ['nome', 'acoes'];
  constructor() {
    effect(() => this.carregar({ ...PAGINACAO_PADRAO, pagina: this.paginaDaRota() }));
  }
  protected abrirFormulario(meioPagamento?: MeioPagamento): void {
    this.dialog
      .open(FormularioMeioPagamentoDialogComponent, {
        data: { meioPagamento },
  ...
  protected excluir(meioPagamento: MeioPagamento): void {
    if (this.excluindo()) {
      return;
    }
    ...
    this.dialog
      ...
      .pipe(
        switchMap((confirmado: boolean | undefined) => {
          ...
          return this.api.inativar(meioPagamento.id).pipe(
...
function paginaVazia(): PaginaResponse<MeioPagamento> {
