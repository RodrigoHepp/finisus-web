...
import { criarParametrosDaPagina, obterPaginaDaRota } from '../../shared/routing/paginacao-da-rota';
...
@Component({
  ...
  styleUrl: './lista-cartoes.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ListaCartoesPage {
  ...
  protected readonly dados = signal<PaginaResponse<Cartao>>(paginaVazia());
  protected readonly carregando = signal(true);
  protected readonly erro = signal<string | null>(null);
  protected readonly excluindo = signal(false);
  protected readonly colunas = ['nome', 'limite', 'fechamento', 'vencimento', 'acoes'];
  constructor() {
    effect(() => this.carregar({ ...PAGINACAO_PADRAO, pagina: this.paginaDaRota() }));
  }
  protected abrirFormulario(cartao?: Cartao): void {
    this.dialog
      .open(FormularioCartaoDialogComponent, {
        data: { cartao },
  ...
  protected excluir(cartao: Cartao): void {
    if (this.excluindo()) {
      return;
    }
    const dados: ConfirmacaoDialogData = {
      titulo: this.translate.instant('CARTOES.CONFIRMACAO_EXCLUSAO.TITULO'),
      mensagem: this.translate.instant('CARTOES.CONFIRMACAO_EXCLUSAO.MENSAGEM', {
        nome: cartao.nome,
      }),
    ...
    this.dialog
      ...
      .pipe(
        switchMap((confirmado: boolean | undefined) => {
          ...
          return this.api.inativar(cartao.id).pipe(
      ...
      .subscribe(() => {
        this.mensagens.sucesso(
          this.translate.instant('CARTOES.MENSAGENS.EXCLUIDO', { nome: cartao.nome }),
        );
        this.carregar({ pagina: this.dados().pagina, tamanho: this.dados().tamanho });
      });
  }
  protected verFaturas(cartao: Cartao): void {
    void this.router.navigate(['/cartoes', cartao.id, 'faturas']);
  }
  ...
  private mensagemDeErro(erro: unknown): string {
    return erro instanceof HttpErrorResponse && typeof erro.error?.detail === 'string'
      ? erro.error.detail
      : this.translate.instant('CARTOES.MENSAGENS.ERRO_CARREGAR');
  }
}
function paginaVazia(): PaginaResponse<Cartao> {
